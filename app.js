...
function renderVideoSlot(exerciseId, exerciseName, exercise = {}) {
  ...
  const activeVideo = uploadedVideo || supabaseVideo || builtInVideo || automaticVideo;
  ...
  return `
    <div class="video-player-shell">
      <video
        ...
        playsinline
        preload="metadata"
        src="${escapeHtml(activeVideo.url)}"
        ${activeVideo.automatic ? `data-auto-video="true" data-exercise-name="${escapeHtml(exerciseName)}" data-exercise-id="${exerciseId}"` : ""}
      ></video>
      <div class="video-meta-row">
        ...
        ${
          ...
          Remove Video
            ...
            : `<span>${supabaseVideo ? "Supabase video" : builtInVideo ? "Built-in demo video" : `Expected file: ${exerciseId}.mp4`}</span>`
        }
      </div>
    </div>
  `;
...
function attachAutomaticVideoFallbacks() {
  document.querySelectorAll('video[data-auto-video="true"]').forEach((video) => {
    video.addEventListener("error", () => {
      const exerciseId = video.dataset.exerciseId;
      const exerciseName = video.dataset.exerciseName || "this exercise";
      ...
      shell.outerHTML = `
        <div class="video-empty-state">
          ...
          <span>Upload an MP4 named <strong>${escapeHtml(exerciseId)}.mp4</strong></span>
        </div>
      `;
    }, { once: true });
  });
